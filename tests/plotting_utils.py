...
def multiple_scatter_plot(y_datas, x_datas, labels, xname='', yname='', title='', colors=('blue', 'red', 'green', 'orange'), yscale='log', regression=False):
    plt.figure()
    fig3, ax3 = plt.subplots()
    for i in range(len(y_datas)):
        x = x_datas[i]
        y = y_datas[i]
        ax3.plot(x, y, color=colors[i], marker="+", ls="", label=labels[i])
        if regression:
